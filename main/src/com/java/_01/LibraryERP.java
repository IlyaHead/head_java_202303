 ...
 * <p>
 * > ADD[ISBN, Title, Author, Year, Price] - Добавить книгу в каталог > REMOVE [ISBN] - Удалить книгу из каталога > PRINT - Вывести информации обо
 * всех книга > Get[ISBN] - Получить информацию о книге по ISBN > GET_BY_AUTHOR [Author] Найти все книги (ISBN) определенного автора > GET_BY_YEAR
 * [YEAR] - найти все книги (ISBN) определенного года издания > GET_TOTAL_PRICE - Получить общую стоимость имеющихся книг > EXIT
 * <p>
...
public class LibraryERP {
    ...
    do {
      ...
      switch (command) {
        case "ADD": {
          ...
          addBook(isbn, tokens[2], tokens[3], Integer.parseInt(tokens[4]),
  ...
  private static void getBookByYear(int year) {
    for (int i = 0; i < countBooks; i++) {
      if (years[i] == year) {
        System.out.println(String.join(",", isbns[i], titles[i], authors[i],
            String.valueOf(years[i]), String.valueOf(prices[i])));
      }
    }
  }
